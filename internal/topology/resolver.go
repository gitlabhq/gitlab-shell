...
const (
	...
	classifyInitialInterval = 100 * time.Millisecond
	classifyMaxInterval     = 500 * time.Millisecond
)
...
func (r *Resolver) Resolve(ctx context.Context, claim *types_proto.Claim) string {
	...
	resp, err := backoff.Retry(ctx, func() (*pb.ClassifyResponse, error) {
		...
		backoff.WithMaxTries(classifyMaxRetries),
		backoff.WithNotify(func(err error, duration time.Duration) {
			slog.WarnContext(ctx, "Topology Service classify attempt failed, retrying",
	...
	if err != nil {
		slog.WarnContext(ctx, "Topology Service classify failed after retries, falling back to default host",
			slog.Int("max_attempts", classifyMaxRetries),
			log.ErrorMessage(err.Error()),
		)
		return ""
	}
