...
func (rt *transport) RoundTrip(request *http.Request) (*http.Response, error) {
	...
	if err != nil {
		logger.Log.InfoContext(ctx, "Internal API unreachable", slog.String(fields.ErrorMessage, err.Error()))
		return response, err
	}

	ctx = log.WithFields(ctx, slog.Int("status", response.StatusCode))
	if response.StatusCode >= 400 {
		logger.Log.ErrorContext(ctx, "Internal API error")
		return response, err
	}
	...
	logger.Log.InfoContext(ctx, "Finished HTTP request")
	return response, nil
}
