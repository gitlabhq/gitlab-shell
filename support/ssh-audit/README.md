...
excludes ChaCha20-Poly1305 and the X25519 key exchanges, keeps the SHA-1
algorithms by default, and offers ML-KEM768/X25519 on Go's native FIPS module
(what CI builds) but not under BoringCrypto. Regenerate on the same backend the
policy targets, so the ML-KEM entry matches.
