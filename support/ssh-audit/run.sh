...
# Pick a free port up front so the listen address is deterministic and we don't
# have to scrape it out of the server logs.
...
for _ in $(seq 1 100); do
  ...
  if ! kill -0 "$SSHD_PID" 2>/dev/null; then
    echo "error: gitlab-sshd exited before it started listening" >&2
